...
def FCRFromBVP(BVP, BCR, RIM, CT):
    ...
    k = 0.001 # units factor
    BCD = (RIA - RIM) / (BCR * k) # Base Curve in diopters
    FCD = (RIM * (BVP - BCD)) / (RIM - BCD * CT * k + BVP * CT * k)  # Front Curve in diopters
...
def find_closest_index(x_list, y_list, point):
    distances = [(x - point[0])**2 + (y - point[1])**2 for x, y in zip(x_list, y_list)]
...
def translate_to_ACS(point, origin, theta):
    ...
    rotation_matrix = np.array([[np.cos(-theta_num), -np.sin(-theta_num)], 
...
def translate_to_GCS(point_acs, origin, theta):
    ...
    rotation_matrix_inv = np.array([[np.cos(theta_num), -np.sin(theta_num)], 
                                    [np.sin(theta_num),  np.cos(theta_num)]])
...
def calc_junction_thickness(Power, Base_radius, RI, Center_Thickness, Front_Optic_Dia, origin_BCCS, angle_BCCS, BC_meridian_x, BC_meridian_y):
    Power = float(Power)
    ...
    angle_FC = angle_BCCS
    FC_sag = float(sagitta(FC_radius_FV, 0, Front_Optic_Dia / 2))  # Saggital depth of the FC zone
    ...
    M_tangent_line_at_end_point_FCCS, _, _ = slope_and_angle_of_line_tangent_to_curve(FC_x_FCCS[-1], FC_radius_FV, FC_K)
    M_normal_line_at_end_point_FCCS = float(-1 / M_tangent_line_at_end_point_FCCS)
    B_normal_line_at_end_point_FCCS = float(y_intercept(FC_end_point_FCCS[0], FC_end_point_FCCS[1], M_normal_line_at_end_point_FCCS))

    point1_on_line_GCS = translate_to_GCS((0, B_normal_line_at_end_point_FCCS), FC_origin_GCS, angle_FC)
    point2_on_line_GCS = translate_to_GCS(FC_end_point_FCCS, FC_origin_GCS, angle_FC)
    
    M_normal_line_at_end_point_GCS = float((point2_on_line_GCS[1] - point1_on_line_GCS[1]) / (point2_on_line_GCS[0] - point1_on_line_GCS[0]))
    B_normal_line_at_end_point_GCS = float(y_intercept(point2_on_line_GCS[0], point2_on_line_GCS[1], M_normal_line_at_end_point_GCS))
    ...
    result = root_scalar(diff_func, bracket=[BC_meridian_x.min(), BC_meridian_x.max()], x0=x_guess)

    if result.converged:
        x_intersect = result.root
        y_intersect = line_func(x_intersect)
        calc_jt = float(distance_between_points((x_intersect, y_intersect), (FC_x_GCS[-1], FC_y_GCS[-1])))
    else:
...
def edge_radius_center(x_i, x_f, R, K, m, b, r, direction):
    ...
    m = float(m) # The slope of the line that is the edge boundary, this must not be verticle that is why this needs to be the line in the CS of the curve tangent to the edge radius
    b = float(b) # The y-intercept of the line that is the edge boundary, this must not be verticle that is why this needs to be the line in the CS of the curve tangent to the edge radius
    ...
    def calculate_offset_line_corr(x, m, b, r, direction):
        d = r * direction
        v_normal = (1 / np.sqrt(1 + (-1/m)**2), (-1/m) / np.sqrt(1 + (-1/m)**2))
        x_o1 = d * v_normal[0]
        y_o1 = b + d * v_normal[1]
        b_o = y_o1 - m * x_o1
        return m * x + b_o
...
def generate_partial_circle_counterclockwise(center_x, center_y, radius, start_x, end_x, num_points=100):
    ...
    if end_angle <= start_angle:
        raise ValueError("The end x value should correspond to a point in the bottom half of the circle.")
...
def generate_partial_circle_clockwise(center_x, center_y, radius, start_x, end_x, num_points=100):
    # Calculate the angle for the starting x value (top half of the circle)
    if np.abs(start_x - center_x) > radius:
        raise ValueError("The starting x value is outside the circle.")
    start_angle = np.arccos((start_x - center_x) / radius)
    ...
    if start_angle <= end_angle:
        raise ValueError("The end x value should correspond to a point lower than the start x value on the circle (for clockwise direction).")
...
def generate_bottom_right_quadrant_of_circle(center_x, center_y, radius, start_x, end_x, num_points=100):
    # Validate that the start_x and end_x are within the bottom right quadrant
    if start_x < center_x or start_x > center_x + radius:
        raise ValueError("The starting x value should be within the bottom right quadrant of the circle.")
    if end_x < center_x or end_x > center_x + radius:
        raise ValueError("The ending x value should be within the bottom right quadrant of the circle.")
    ...
    if not (3 * np.pi / 2 <= start_angle <= 2 * np.pi):
        raise ValueError("The start x value should correspond to an angle between 270° and 360°.")
    if not (3 * np.pi / 2 <= end_angle <= 2 * np.pi):
        raise ValueError("The end x value should correspond to an angle between 270° and 360°.")
...
def cubic_interpolate(x_points, y_points, x):
    ...
    interpolating_function = interp1d(x_points, y_points, kind='cubic', fill_value="extrapolate")
...
def compute_tangent_line_b(x_val, x_points, y_points):
    ...
    selected_indices = indices[:30]  # Select 30 closest points (15 before and 15 after x_val)
    ...
    slope_tangent = (interpolator(x_val + delta) - interpolator(x_val - delta)) / (2 * delta)
...
def compute_normal_line(x_val, x_points, y_points):
    ...
    if slope_tangent != 0:
        ...
        slope_normal = float('inf')  # Handle the case where the tangent line is horizontal
...
def truncate_points(x0, y0, x1, y1):
    ...
    distances = [np.sqrt((x - first_point_x1)**2 + (y - first_point_y1)**2) for x, y in zip(x0, y0)]
...
def truncate_and_interpolate(x_list, y_list, x_truncate):
    ...
    for i in range(len(x_list) - 1):
        ...
        if x_list[i] <= x_truncate <= x_list[i + 1]:
            ...
            y_truncate = y_list[i] + (y_list[i + 1] - y_list[i]) * (x_truncate - x_list[i]) / (x_list[i + 1] - x_list[i])
            #print(f"Interpolated y_truncate: {y_truncate}")
            
            # Truncate the lists up to the desired point
            x_truncated = x_list[:i + 1] # + [x_truncate]  # Explicitly create a new list up to the truncation point
...
def blend_curves(x1, y1, x2, y2, blend_radius):
    ...
    if distance > 0.01:
        raise ValueError("The end point of the first curve and the start point of the second curve are too far apart.")
    ...
    if np.abs(delta_x) < 1e-6:  # Handle the case where delta_x is zero or close to zero
        ...
        hermite_spline = CubicHermiteSpline([x1[-1], x2[0]], [y1[-1], y2[0]], [slope1, slope2])
...
def blend_curves2(x1, y1, x2, y2, blend_radius, d):
    ...
    if np.abs(delta_x) < 1e-6:  # Handle the case where delta_x is zero or close to zero
        ...
        hermite_spline = CubicHermiteSpline([x1_blend_start, x2_blend_end], [y1_blend_start, y2_blend_end], [slope1, slope2])
        blend_y = hermite_spline(blend_x)

    # Concatenate the curves
    x_blended = np.concatenate((x1[:x1_blend_start_index], blend_x, x2[x2_blend_end_index:]))
    y_blended = np.concatenate((y1[:x1_blend_start_index], blend_y, y2[x2_blend_end_index:]))
...
def write_b_side_file(Header, Surface, points_x, points_y, filename):
    ...
    with open(full_filename, 'w') as file:
        ...
        for key, value in Surface.items():
            ...
            if Header["non_rotationally_symmetrical"]["value"] == 0 and key in ["num_radial_defs", "rotation_angle"]:
    ...
    with open(full_filename, 'w') as file:
        ...
        for idx, (sublist_x, sublist_y) in enumerate(zip(points_x, points_y)):
            ...
            if len(sublist_x) != len(sublist_y):
                raise ValueError(f"Mismatch in number of points between x and y sublists at index {idx}.")
            ...
            for x, y in zip(reversed(sublist_x), reversed(sublist_y)):
                ...
                y_standard = f"{y:.10f}".rstrip('0').rstrip('.') if 'e' in f"{y}" or 'E' in f"{y}" else f"{y:.10f}".rstrip('0').rstrip('.')
...
def write_f_side_file(Header, Surface, points_x, points_y, filename):
    ...
    with open(full_filename, 'w') as file:
        ...
        for key, value in Surface.items():
            ...
            # Prepend a "\" if the condition is met
            if Header["non_rotationally_symmetrical"]["value"] == 0 and key in ["angular_filtering", "num_radial_defs", "rotation_angle"]:
                line = f"\\{value_str:<19}\\ {description_str}"
            else:
                line = f"{value_str:<20}\\ {description_str}"
            file.write(line + "\n")
    ...
    with open(full_filename, 'w') as file:
        ...
        for idx, (sublist_x, sublist_y) in enumerate(zip(points_x, points_y)):
            ...
            for x, y in zip(reversed(sublist_x), reversed(adjusted_points_y)):
                # Convert y to standard decimal notation if needed
                y_standard = f"{y + last_y_value:.10f}".rstrip('0').rstrip('.') if 'e' in f"{y}" or 'E' in f"{y}" else f"{y + last_y_value:.10f}".rstrip('0').rstrip('.')
    ...
    with open(full_filename, 'w') as file:
        ...
        if num_diag_marks > 0:
            for mark in Header["diag_marks"]:
                ...
                file.write(f"{mark['distance_from_edge']:<20}\\ distance from edge {mark['note']}\n")
                file.write(f"{mark['length']:<20}\\ length             {mark['note']}\n")
...
def segment_curve(x_values, y_values, start_x):
    ...
    if x[start_index] == start_x:
        ...
        if start_index == 0:  # If start_x is less than the first x, use the first y
            ...
            slope = (y[start_index] - y[start_index - 1]) / (x[start_index] - x[start_index - 1])
